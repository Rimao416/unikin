...
if(isset($_POST["action"]))
{
 if($_POST["action"] == 'fetch')
 {
  ...
  foreach($result as $row)
  {
   ...
   $sub_array[] = '<button type="button" name="edit_grade" class="btn btn-primary btn-sm edit_grade" id="'.$row["grade_id"].'">Modifier</button>';
 ...
 if($_POST["action"] == 'Add' || $_POST["action"] == "Edit")
 {
  ...
  {
   $error_grade_name = 'Le grade de la casse est requise';
   $error++;
  }
  else
  {
   $grade_name = strtoupper($_POST["grade_name"]);
  }
  if($error > 0)
  {
   $output = array(
  ...
  else
  {
   if($_POST["action"] == "Add")
   {
    ...
    $query = "INSERT INTO tbl_grade (grade_name) SELECT * FROM (SELECT :grade_name) as temp 
    WHERE NOT EXISTS (
    ...
    if($statement->execute($data))
    {
     ...
     else
     {
      $output = array(
       'error'     => true,
       'error_grade_name' => 'Ce grade existe déjà'
      );
     }
    }
   }

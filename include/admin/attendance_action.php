<?php
    ...
    if(isset($_POST["action"])){
        if($_POST["action"]=="fetch"){
            $query="SELECT * FROM tbl_attendance INNER JOIN tbl_student
                ON tbl_student.student_id = tbl_attendance.student_id 
                INNER JOIN departement
                ON departement.id = tbl_student.student_dep
                ...
                if(isset($_POST["order"])){
                    ...
                    echo json_encode($output);
                        ...
                        if($_POST["action"]=="index_fetch"){
                            $query="SELECT * FROM tbl_student LEFT JOIN tbl_attendance
                        ...
                        foreach($result as $row)
                            {
                                ...
                                $sub_array[] = $row["student_name"];
                                $sub_array[] = $row["student_roll_number"];
                                $sub_array[] = get_attendance_percentage_admin($connect, $row["student_id"]);
                                $sub_array[] = '<button type="button" name="report_button" data-student_id="'.$row["student_id"].'" class="btn btn-info btn-sm report_button">Rapport PDF</button>&nbsp;&nbsp;&nbsp;<button type="button" name="chart_button" data-student_id="'.$row["student_id"].'" class="btn btn-danger btn-sm report_button">Graphique</button>';
                                $data[] = $sub_array;
                            }
                            $output = array(
                                'draw'    => intval($_POST["draw"]),
